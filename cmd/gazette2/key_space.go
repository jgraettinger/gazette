...
func newBrokerKeySpace(prefix string) *keyspace.KeySpace {
...
func (d decoder) DecodeItem(id string, raw *mvccpb.KeyValue) (v3_allocator.ItemValue, error) {
	var s = new(protocol.JournalSpec)
	...
	if err := s.Unmarshal(raw.Value); err != nil {
		...
		return nil, protocol.NewValidationError("JournalSpec Name doesn't match Item ID (%+v vs %+v)", s.Name, id)
	}
	return s, nil
}

func (d decoder) DecodeMember(zone, suffix string, raw *mvccpb.KeyValue) (v3_allocator.MemberValue, error) {
	var s = new(protocol.BrokerSpec)
	...
	if err := s.Unmarshal(raw.Value); err != nil {
		...
		return nil, protocol.NewValidationError("BrokerSpec Zone doesn't match Member Zone (%+v vs %+v)", s.Id.Zone, zone)
	} else if s.Id.Suffix != suffix {
		return nil, protocol.NewValidationError("BrokerSpec Suffix doesn't match Member Suffix (%+v vs %+v)", s.Id.Suffix, suffix)
	}
	return s, nil
}
...
func (d decoder) DecodeAssignment(itemID, memberZone, memberSuffix string, slot int, raw *mvccpb.KeyValue) (v3_allocator.AssignmentValue, error) {
	var s = &protocol.Route{Primary: -1}
